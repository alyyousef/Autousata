...
exports.createDirectPaymentIntent = async (req, res) => {
    ...
    try {
        ...
        `;
        const existingPayment = await connection.execute(existingPaymentQuery, { vehicleId });
...
exports.confirmPayment = async (req, res) => {
    ...
    try {
        ...
        if (paymentResult.rows.length > 0) {
            ...
            if (payment.status === 'completed') {
                const fetchQuery = `
                    ...
                    WHERE p.ID = :paymentId
                `;
                const existingResult = await connection.execute(fetchQuery, { paymentId: payment.id });
            ...
            paymentIntent = await stripe.paymentIntents.retrieve(clientGatewayOrderId, {
                expand: ['charges.data.balance_transaction']
            });
        ...
        const bidAmount = parseInt(meta.vehiclePrice || meta.bidAmount) || totalAmount;
        ...
        const paymentDetailsResult = await connection.execute(fetchQuery, { paymentId: payment.id });
