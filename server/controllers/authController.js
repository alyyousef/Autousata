...
async function login(req, res) {
  ...
  try {
    ...
    if (authData.status === "UNVERIFIED")
      return res
        .status(403)
        .json({ error: "Please verify your email.", needsVerification: true });
...
async function getMe(req, res) {
  ...
  try {
    ...
    // 🔍 DEBUG LOG
    console.log(
      `🔄 GetMe: ${dbUser.EMAIL} | Status: ${dbUser.KYC_STATUS} | Addr: ${dbUser.KYC_ADDRESS_FROM_ID ? "Yes" : "No"}`,
    );
    ...
    res.json({
      user: {
        ...
        profileImage: dbUser.PROFILE_PIC_URL,
        emailVerified:
          dbUser.EMAIL_VERIFIED == "1" || dbUser.EMAIL_VERIFIED == 1,
        phoneVerified:
          dbUser.PHONE_VERIFIED == "1" || dbUser.PHONE_VERIFIED == 1,
