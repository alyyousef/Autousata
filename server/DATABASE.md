...
### 11. USER_MODERATION_LOGS

| Column       | Type           | Constraints               | Description                                                 |
| ------------ | -------------- | ------------------------- | ----------------------------------------------------------- |
| `ID`         | VARCHAR2(36)   | PK                        | Unique log entry ID (UUID via `SYS_GUID`)                   |
| `USER_ID`    | VARCHAR2(101)  | FK → USERS(ID)            | The user being moderated                                    |
| `ADMIN_ID`   | VARCHAR2(101)  | FK → USERS(ID)            | Admin who performed the action                              |
| `ACTION`     | VARCHAR2(30)   | Check                     | Moderation action (`ban`, `unban`, `suspend`, `reactivate`) |
| `REASON`     | VARCHAR2(1000) |                           | Human-readable reason for the action                        |
| `EVIDENCE`   | CLOB           | JSON (optional)           | Supporting evidence (links, notes, screenshots, metadata)   |
