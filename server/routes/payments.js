...
router.post(
  ...
  activityLogger({
    ...
    severity: "INFO",
    entityType: "PAYMENT",
    getDescription: (req) => `Create payment intent (auction winner)`,
  }),
...
router.post(
  ...
  authenticate,
  activityLogger({
    action: "PAYMENT_DIRECT_INTENT_CREATE",
    severity: "INFO",
    entityType: "PAYMENT",
    getDescription: (req) =>
      `Create direct payment intent for vehicle ${req.body?.vehicleId || "unknown"}`,
  }),
...
router.post(
  ...
  activityLogger({
    ...
    getDescription: (req) => `Confirm payment ${req.params.id}`,
  }),
...
router.get(
  "/auction/:auctionId",
  authenticate,
  paymentController.getPaymentByAuction,
);
...
router.post(
  ...
  activityLogger({
    ...
    getEntityId: (req) => req.params.id,
    getDescription: (req) =>
      `Buyer confirmed receipt for escrow ${req.params.id}`,
  }),
  async (req, res) => {
    const db = require("../config/db");
    const connection = await db.getConnection();
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (escrow.status === "released" || escrow.status === "refunded") {
        return res
          .status(400)
          .json({ error: "Cannot dispute completed escrow" });
      }
